...
def is_financial_sms(message: str) -> bool:
    ...
    # Ignore VIT/student messages with registration numbers (not actual transactions)
    vit_patterns = [
        r'dear\s+student.*\d{10}[a-z0-9]+',  # VIT registration patterns
        r'reference\s+number.*\d{10}[a-z0-9]+',  # Reference number patterns
        r'check\s+receipt.*vt\s*op',  # VTOP receipt messages
        r'bce\d+',  # BCE codes
        r'23bce\d+',  # Specific BCE pattern
    ]
